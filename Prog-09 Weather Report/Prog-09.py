...
def approx_match(stations, name):
    name_temp = ''.join([i.strip().upper() for i in name])
    return [i for i in stations if name_temp in ''.join([j.strip().upper() for j in i.upper()])]

def top_k_min_temp_stations(stations, K):
    return [i[1] for i in sorted([[stations[i]['temp'],i] for i in stations])[:K]]

def top_k_max_temp_stations(stations, K):
    return [i[1] for i in sorted([[-stations[i]['temp'],i] for i in stations])[:K]]

def peak_stations(stations):
    min_lat = [[stations[i]['lat'],i] for i in stations]
    max_lat = [[-stations[i]['lat'],i] for i in stations]
    min_long = [[stations[i]['long'],i] for i in stations]
    max_long = [[-stations[i]['long'],i] for i in stations]
    return [sorted(min_lat)[0][1],sorted(max_lat)[0][1],sorted(min_long)[0][1],sorted(max_long)[0][1]]

def k_nearby_stations(stations, main_station, K):
    return [i[1] for i in sorted([[distance(stations[main_station]['lat'],stations[main_station]['long'],\
        stations[i]['lat'],stations[i]['long']),i] for i in stations])[1:K+1]]

def average_temp(stations, names):
    temp = 0
    for i in names:
        temp += stations[i]['temp'] 
    return temp/len(names)
