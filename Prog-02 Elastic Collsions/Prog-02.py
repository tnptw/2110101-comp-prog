...
def update_v(x1, y1, r1, v1x, v1y, 
    ...
    seta_1 = math.atan2(v1y , v1x)
    seta_2 = math.atan2(v2y , v2x)
    v1 =  math.sqrt(v1x**2+v1y**2)
    v2 = math.sqrt(v2x**2+v2y**2)
    
    v1_x = ((((v1*math.cos(seta_1-phi)*(m1-m2))+(2*m2*v2*math.cos(seta_2-phi)))/(m1+m2))*math.cos(phi))+(v1*math.sin(seta_1-phi)*math.cos(phi+(math.pi/2)))
    v1_y = ((((v1*math.cos(seta_1-phi)*(m1-m2))+(2*m2*v2*math.cos(seta_2-phi)))/(m1+m2))*math.sin(phi))+(v1*math.sin(seta_1-phi)*math.sin(phi+(math.pi/2)))
    v2_x = ((((v2*math.cos(seta_2-phi)*(m2-m1))+(2*m1*v1*math.cos(seta_1-phi)))/(m1+m2))*math.cos(phi))+(v2*math.sin(seta_2-phi)*math.cos(phi+(math.pi/2)))
    v2_y = ((((v2*math.cos(seta_2-phi)*(m2-m1))+(2*m1*v1*math.cos(seta_1-phi)))/(m1+m2))*math.sin(phi))+(v2*math.sin(seta_2-phi)*math.sin(phi+(math.pi/2)))
